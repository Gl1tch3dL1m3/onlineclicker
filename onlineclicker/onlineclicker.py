...
async def execDB(mysql: str, vars: tuple = None, sqlite: str = None) -> list:
    ...
    Parameters:
        ...
        mysql (str): *Optional.* SQLite query to execute.
...
class Server:
    ...
    def nodes(self) -> dict[str, list[str]]:
        """List of every node and usernames of players connected to them."""
        return self._nodes

    def __repr__(self):
        attrs = " ".join(f"{k}={("'" + v + "'") if isinstance(v, str) else v}" for k, v in self.__dict__.items() if not hasattr(v, "__call__") and not k.startswith("_"))
    ...
    def get_player_by_username(self, username: str, remove_sensitive: bool = True) -> Player:
        ...
        for player in self.all_players.copy().values():
            if username == player.username:
                if remove_sensitive:
                    return player.safe().to_json()
                else:
                    return player
            
        return False

    async def send_chat(self, node: str, message: Message, log_message: bool = False) -> None:
    ...
    async def broadcast_to_node(self, node: str, message: dict, sender_websocket: websockets.ServerConnection = None, username: str = None, send_to_sender: bool = True) -> None:
        ...
        **IMPORTANT:** `sender_websocket` and `username` are optional, but at least one of them **must** be given!
        ...
        Parameters:
            ...
            username (str): *Optional.* Username of the player that broadcast_to_nodebroadcasted the message. If set, the value will be included in the JSON message.
        ...
        for websocket, player in self.all_players.items():
            if player.node == node and (sender_websocket == None or ((not send_to_sender and (websocket != sender_websocket and username != player.username)) or send_to_sender)):
                try:
    ...
    async def __validate_players(self):
        while True:
            ...
            if len(accounts) != 0:
                accounts = {row[0].lower(): row[1] for row in accounts}

            now = datetime.datetime.now()

            for websocket, player in self.all_players.copy().items():
                try:
                    if player.password != accounts[player.username.lower()]:
                        await self.kick_player_by_websocket(websocket, "Your password has been changed. Please log in again.")
                    ...
                    await self.kick_player_by_websocket(websocket, "Internal server error. If this keeps happening, please join our Discord and report it. Sorry!")
            ...
            async for req in websocket:
                ...
                if "request" not in json_req:
                    ...
                    if not (websocket in _chat_ratelimit and datetime.datetime.now() - _chat_ratelimit[websocket] < datetime.timedelta(seconds=2)):
                        ...
                        if player.username in self.owners or player.discord_id in self.owners:
                            badges.append(Badge.OWNER) # owner badge
                        if player.username in self.moderators or player.discord_id in self.moderators:
                            badges.append(Badge.MODERATOR) # moderator badge
                        if player.username in self.verified or player.discord_id in self.verified:
                            badges.append(Badge.VERIFIED) # verified badge
                        if player.username in self.supporters or player.discord_id in self.supporters:
                        ...
                        if allowed_to_send == None or allowed_to_send:
                            await self.send_chat(player.node, message, log_message=True)
...
class SafePlayer:
    """Safe version of Player (without last_heartbeat, websocket, password)."""
    ...
    def __init__(self,
        ...
        """
        Parameters:
            discord_id (int): Player Discord user ID.
            node (str): Node to which the player is assigned.
            username (str): Player username.
            position (PlayerPosition): Player position object.
            flip (bool): Whether the player sprite is flipped (facing right).
            status (PlayerStatus): Player status object.
            nickname_color (NicknameColor): Player nickname color object.
            statistics (PlayerStatistics): Player statistics object.
        """
...
class Player(SafePlayer):
    """A class representing a player connected to the server. Derived from SafePlayer."""
    ...
    def __init__(self,
        ...
        """
        Parameters:
            websocket (websockets.ServerConnection): Connection object of the player.
            discord_id (int): Player Discord user ID.
            node (str): Node to which the player is assigned.
            username (str): Player username.
            password (str): Player hashed password.
            last_heartbeat (datetime.datetime): Last time the player sent a heartbeat message.
            position (PlayerPosition): Player position object.
            flip (bool): Whether the player sprite is flipped (facing right).
            status (PlayerStatus): Player status object.
            nickname_color (NicknameColor): Player nickname color object.
...
class Badge(Enum):
    ...
    OWNER: int = 0
    """This attribute has a value of 0. It represents a red badge with two hammers."""
    MODERATOR: int = 1
    """This attribute has a value of 1. It represents a green badge with a sword."""
    VERIFIED: int = 2
    """This attribute has a value of 2. It represents a purple badge with a checkmark."""
    SUPPORTER: int = 3
